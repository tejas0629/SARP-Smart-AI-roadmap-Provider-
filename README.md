...
Roadmap topics are generated by Gemini. For each topic, Django uses Serper to retrieve current web and video search results, then Gemini selects one suitable website page and one specific YouTube video from those results. React displays the selected resources as clickable icons that open the exact returned URLs in a new tab. Set `SERPER_API_KEY` only in the backend `.env`; it is never sent to the frontend.

Run the backend tests with `python manage.py test`. Serper failures or a missing key leave the normal roadmap available without study-material links.
